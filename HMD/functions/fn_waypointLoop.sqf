...
while { APD_HMD_WaypointLoopRunning } do
{
	...
	if (count _waypoint > 0) then
	{
		...
		_vehicle setUserMFDText [15, format ["%1 %2", _distanceValue, _distanceUnit]];
		_vehicle setUserMFDText [16, format ["T-%1:%2", _etaMinute, _etaSecond]];

		sleep 0.5;
	}
	else
	{
		_vehicle setUserMFDText [15, ""]; // Waypoint Distance
		_vehicle setUserMFDText [16, ""]; // Waypoint ETA
